...
function Player:init()
	self.states = {}
	self.states["run"] = AnimatedSprite.new(Texture.new("assets/player_run.png"), 16, 23)
	self.states["jump"] = Bitmap.new(Texture.new("assets/player_jump.png"))
	self.states["dead"] = Bitmap.new(Texture.new("assets/player_dead.png"))
	self.states["jet"] = Bitmap.new(Texture.new("assets/player_jetpack.png"))
	for k,v in pairs(self.states) do
		v:setX(-3)
	end
	self.currentState  = "run"
	self:addChild(self.states["run"])
	--self:addChild(Bitmap.new(Texture.new("assets/player.png")))
	self.sx = 0
	self.sy = 0
	self.px = 0
	self.py = 0
